...
class GRUClass(object):
    ...
    def lstm_cell(self):
        ...
        return tf.nn.rnn_cell.DropoutWrapper(cell=gru_cell, output_keep_prob=1-self.placeholders['dropout'])

    def encoder(self):
        '''
        :return:
        '''
        self.mgru_cell = tf.nn.rnn_cell.MultiRNNCell([self.lstm_cell() for _ in range(self.layer_num)], state_is_tuple=True)
        self.initial_state=self.mgru_cell.zero_state(self.batch_size,tf.float32)
    ...
    def encoding(self, inputs=None):
        ...
        initial_state=self.initial_state
        with tf.variable_scope('encoder', reuse=tf.AUTO_REUSE):
            h_states, c_states = tf.nn.dynamic_rnn(cell=self.mgru_cell, inputs=inputs, initial_state=initial_state, dtype=tf.float32)
        return (h_states,c_states)