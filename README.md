...
We have added all the baseline models results and codes to our GitHub, inluding ST-GRAT, GMAN, AST-GAT, DCRNN, T-GCN, Etc! Note, we did not add the ST-GRAT and DCRNN results to our paper because the prediction precision of DCRNN and ST-GRAT (implemented by PyTorch, torch==1.6.0) is further lower than GMAN, but we give the results on the dataset as the following shows. If you need, we can added both to paper!!!  
