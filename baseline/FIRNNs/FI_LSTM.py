...
class FirnnClass(object):
    ...
    def decoding(self,  encoder_hs, site_num):
        ...
        for i in range(self.predict_time):
            ...
            results = tf.layers.dense(inputs=h_state, units=64, activation=tf.nn.relu, name='layer_speed_1', reuse=tf.AUTO_REUSE)
            results = tf.layers.dense(inputs=results, units=1, name='layer_speed_2', reuse=tf.AUTO_REUSE)
