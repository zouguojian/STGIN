...
class DataClass(object):
    ...
    def generator(self):
        ...
        if self.is_training:
            low, high = 7*2*24*4, int(self.shape[0]//self.site_num * self.divide_ratio)
        else:
        ...
        while low + self.input_length + self.output_length <= high:
            ...
            yield (data[(low-7*2*24*4) * self.site_num : (low-7*2*24*4 + self.input_length) * self.site_num, 5:6],
                   data[(low - 7 * 24 * 4+ self.input_length) * self.site_num: (low - 7 * 24 * 4 + self.input_length*2) * self.site_num,5:6],
